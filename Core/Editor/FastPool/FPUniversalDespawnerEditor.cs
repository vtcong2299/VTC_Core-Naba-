...
namespace NabaGame.Core.Editor.Pool
{
    ...
    public class FPUniversalDespawnerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.PropertyField(targetPoolID,
                new GUIContent("Target Pool ID", "ID of the pool to despawn this game object into"));
            GUILayout.Space(5);
            GUILayout.Label("Despawn conditions [OR]", EditorStyles.objectFieldThumb);

            EditorGUILayout.BeginVertical("helpBox");
            EditorGUILayout.PropertyField(despawnDelayed,
                new GUIContent("Delay", "Despawn the object after the specified period of time"));

            if (despawnDelayed.boolValue)
            {
                EditorGUILayout.PropertyField(delay,
                    new GUIContent("(seconds)", "Despawn the object after the specified period of time"));
            ...
            EditorGUILayout.PropertyField(despawnOnParticlesDead,
                new GUIContent("Particles Dead", "Despawn the object after the particles on this GameObject will die"));

            if (despawnOnParticlesDead.boolValue)
            {
                EditorGUILayout.PropertyField(resetParticleSystem,
                    new GUIContent("Reset On Despawn",
                        "Reset particle system on despawn. Useful when the game object will be despawned before than all particles will die"));
                GUILayout.Space(5);
            }

            EditorGUILayout.PropertyField(despawnOnAudioSourceStop,
                new GUIContent("AudioSource Stop",
            ...
            if (despawnDelayed.boolValue && Mathf.Approximately(delay.floatValue, 0))
                EditorGUILayout.HelpBox("Delay is set to '0' so gameObject will be despawned immediately after spawn!",
                    MessageType.Warning);

            if (despawnOnParticlesDead.boolValue && !haveParticles)
                EditorGUILayout.HelpBox("You don't have any particle system on current or child GameObjects",
                    MessageType.Error);

            if (despawnOnAudioSourceStop.boolValue && !haveAudioSources)
                EditorGUILayout.HelpBox("You don't have any AudioSource on current or child GameObjects",
                    MessageType.Error);
