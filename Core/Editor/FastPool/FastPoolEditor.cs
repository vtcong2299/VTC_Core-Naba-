...
namespace NabaGame.Core.Editor.Pool
{
    ...
    public class FastPoolEditor : PropertyDrawer
    {
        ...
        string displayName = "No Source Prefab";
            ...
            {
                EditorGUI.LabelField(propRect,
                    string.Concat(sourcePrefab != null ? sourcePrefab.name : displayName, " Pool"),
                ...
                EditorGUI.ProgressBar(propRect, (float) spCached.intValue / spCapacity.intValue,
                    string.Concat(spCached.intValue.ToString(), "/",
                        spCapacity.intValue > 0 ? spCapacity.intValue.ToString() : "Unlimited"));
            }
            else
            {
                ...
                Rect bgRect = new Rect(position.x, position.y + EditorGUIUtility.singleLineHeight, position.width,
                ...
                if (GUI.Button(buttonRect,
                    string.Concat((property.isExpanded ? "▼ " : "▶ "),
                        (sourcePrefab != null ? sourcePrefab.name : displayName),
                        (useCustomIDProp.boolValue ? string.Concat(" [Custom ID: ", customIDProp.intValue, "]") : "")),
                    EditorStyles.objectFieldThumb))
                ...
                if (property.isExpanded)
                {
                    ...
                    if (property.boolValue)
                    {
                        ...
                        EditorGUI.PropertyField(customIDRect, property,
                            new GUIContent("ID",
                                "Custom pool ID. By default it equals to the InstanceID of the source prefab."));
                        EditorGUI.indentLevel = 1;
                    ...
                    else
                    {
                        ...
                        if (despawner == null)
                        {
                            ...
                            if (GUI.Button(propRect, "Add Auto Despawner", EditorStyles.miniButton))
                            {
                                ...
                                PoolItemNotificationType notificationType =
                                    (PoolItemNotificationType) notificationTypeProp.enumValueIndex;
                                if (notificationType == PoolItemNotificationType.None ||
                                    notificationType == PoolItemNotificationType.Interface)
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (Application.isPlaying)
                return (EditorGUIUtility.singleLineHeight +
                        (property.isExpanded ? EditorGUIUtility.standardVerticalSpacing : 0)) * 2;
            else
                return (property.isExpanded ? despawnerHeight : 0) +
                       (EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing) *
                       property.CountInProperty();
        }


        void DrawFold(SerializedProperty property, string caption, Rect buttonRect, Rect contentRect,
            Action drawContent)
        {
