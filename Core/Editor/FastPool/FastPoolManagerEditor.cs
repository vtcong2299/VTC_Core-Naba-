        ...
        public override void OnInspectorGUI()
        {
            ...
            if (EditorApplication.isPlaying)
            {
                foreach (KeyValuePair<int, FastPool> pool in fpm.Pools)
                {
                    ...
                    {
                        EditorGUILayout.LabelField(string.Format("{0} [ID {1}]", pool.Value.Name, pool.Value.ID),
                            EditorStyles.objectFieldThumb);

                        Rect rect = GUILayoutUtility.GetRect(18, 18, "TextField");
                        EditorGUI.ProgressBar(rect, (float) pool.Value.Cached / pool.Value.Capacity,
                            string.Concat(pool.Value.Cached.ToString(), "/",
                                pool.Value.Capacity > 0 ? pool.Value.Capacity.ToString() : "Unlimited"));
            ...
            else
            {
                    ...
                    if (dragOk)
                    {
                        ...
                        if (eventType == EventType.DragPerform)
                        {
                            for (int i = 0; i < DragAndDrop.objectReferences.Length; i++)
                            {
                                ...
                                pools.GetArrayElementAtIndex(pools.arraySize - 1).FindPropertyRelative("sourcePrefab")
                                    .objectReferenceValue = DragAndDrop.objectReferences[i];
                                pools.GetArrayElementAtIndex(pools.arraySize - 1).FindPropertyRelative("customID")
                                    .intValue = GetRandomCustomPoolID();
                            }
                ...
                {
                    pools.InsertArrayElementAtIndex(pools.arraySize);
                    pools.GetArrayElementAtIndex(pools.arraySize - 1).FindPropertyRelative("customID").intValue =
                        GetRandomCustomPoolID();
                }
