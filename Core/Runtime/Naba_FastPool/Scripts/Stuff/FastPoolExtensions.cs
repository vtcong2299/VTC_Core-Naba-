...
namespace NabaGame.Core.Runtime.Pool
{
    public static class FastPoolExtensions
    {
        ...
        public static GameObject FastInstantiate(this GameObject sourcePrefab, Vector3 position, Quaternion rotation,
            Transform parentTransform = null)
        {
            return FastPoolManager.GetPool(sourcePrefab, true).FastInstantiate(position, rotation, parentTransform);
        }

        public static GameObject FastInstantiate(this GameObject sourcePrefab, int poolID,
            Transform parentTransform = null)
        {
            return FastPoolManager.GetPool(poolID, sourcePrefab, true).FastInstantiate(parentTransform);
        }

        public static GameObject FastInstantiate(this GameObject sourcePrefab, int poolID, Vector3 position,
            Quaternion rotation, Transform parentTransform = null)
        {
            return FastPoolManager.GetPool(poolID, sourcePrefab, true)
                .FastInstantiate(position, rotation, parentTransform);
        }
