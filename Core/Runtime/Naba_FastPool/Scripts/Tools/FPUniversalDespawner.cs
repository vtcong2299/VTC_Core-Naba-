...
namespace NabaGame.Core.Runtime.Pool
{
    public class FPUniversalDespawner : MonoBehaviour, IFastPoolItem
    {
        ...
        [SerializeField] int targetPoolID;
        [SerializeField] bool despawnDelayed = false;
        [SerializeField] float delay = 0;
        [SerializeField] bool despawnOnParticlesDead = false;
        [SerializeField] bool resetParticleSystem = false;
        [SerializeField] bool despawnOnAudioSourceStop = false;
        ...
        IEnumerator Despawn(float despawn_delay)
        {
            ...
            if (FastPoolManager.Instance != null)
                ...
                Debug.LogError(
                    "FastPoolManager is not present in the scene or being disabled! AutoDespawn will not working on GameObject " +
                    name);
        }
